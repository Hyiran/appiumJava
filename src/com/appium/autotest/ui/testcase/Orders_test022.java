...
public class Orders_test022 extends initializerValue{
	...
	// 马桶疏通下单
	@Test
	public void twenty_twotest() throws Exception, InterruptedException {
		try {
			commonCleaning.CheckHome(driver);
			FirstaidService.Firstaid("马桶疏通", driver);
			FirstaidService.Reservation("马桶疏通", driver);
			FirstaidService.confirmation(driver);
			FirstaidService.timeSlot(driver);
			FirstaidService.payMode(driver);
			FirstaidService.checkClosestool(driver);
